...
const char * const DEEPCOIN_CONF_FILENAME = "deepcoin.conf";
const char * const DEEPCOIN_PID_FILENAME = "deepcoind.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "deepcoind -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Deepcoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Deepcoin
    // Mac: ~/Library/Application Support/Deepcoin
    // Unix: ~/.deepcoin
#ifdef WIN32
    // Windows
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Deepcoin";
#else
    // Unix
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", DEEPCOIN_CONF_FILENAME);
...
fs::path GetPidFile()
{
    return AbsPathForConfigVal(fs::path(gArgs.GetArg("-pid", DEEPCOIN_PID_FILENAME)));
}
