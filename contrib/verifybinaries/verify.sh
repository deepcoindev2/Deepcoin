...
###   deepcoincore.org and deepcoin.org and compares them.
...
WORKINGDIR="/tmp/deepcoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://deepcoincore.org"
HOST2="https://deepcoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="deepcoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'deepcoin-',
...
if [ $? -ne 0 ]; then
   echo "deepcoin.org failed to provide signature file, but deepcoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "deepcoin.org and deepcoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Deepcoin Core binary release signing key installed?"
   fi
